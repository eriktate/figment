...
pub fn main() !void {
    ...
    var pl_ctx = try pipeline.Context.init(alloc);
    defer pl_ctx.deinit();

    log.info("process sprites", .{});
    try pipeline.processFolder(&pl_ctx, "./assets/sprites/");

    log.info("write bitmap", .{});
    try pipeline.writeBitmap(pl_ctx, "./assets/sprites/atlas.png");
    log.info("generate asset code", .{});
    try pipeline.genAnimationCode(pl_ctx);
